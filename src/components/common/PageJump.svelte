<script lang="ts">
	...
	const { variant, currentPage, lastPage, onJump, hrefTemplate, hrefFirst }: Props = $props();
	...
	function jump(page: number) {
		...
		const href = page === 1 && hrefFirst ? hrefFirst : hrefTemplate.replace("{page}", String(page));
...
{#if variant === "ellipsis"}
	...
	{:else}
		<button
			...
			onclick={open}
		>
			<svg class="w-11 h-11" width="24" height="24" viewBox="0 0 24 24" aria-hidden="true"
				><path
					fill="currentColor"
