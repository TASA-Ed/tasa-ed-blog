<script lang="ts">
	...
	const search = async (keyword: string, input: "desktop" | "mobile"): Promise<void> => {
...
>
  ...
  "
	...
	{:else if result.length > 0}
           ...
           rounded-xl text-lg px-3 py-2 hover:bg-(--btn-plain-bg-hover) active:bg-(--btn-plain-bg-active)"
			>
				...
				{#if item.content && item.content.includes("<mark>")}
					...
					>
						<span
							style="display: inline-block; background-color: var(--btn-plain-bg-active); color: var(--primary); padding: 0.1em 0.4em; border-radius: 5px; font-size: 0.75em; font-weight: 600; margin-right: 0.5em; shrink: 0;"
						>
		...
		{#if result.length > 5}
			<a
				href={getSearchUrl(activeKeywordTrimmed)}
