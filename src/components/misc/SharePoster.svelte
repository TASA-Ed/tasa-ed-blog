<script lang="ts">
	...
	function resolveSiteLogoSource(color: string, size: number): string | null {
		...
		if (logo.type === "icon") {
			...
			return serializeNavbarIcon(color, size) ?? buildIconDataUrl(logo.value, color);
		}
	...
	async function generatePoster() {
		...
		try {
			...
			const siteTitleText = fitText(
				ctx,
				siteTitle,
				contentWidth - (siteTitleX - padding) - (dateWidth > 0 ? dateWidth + 16 * scale : 0),
			);
...
{#if showModal}
	...
	>
		<div
			class="bg-white dark:bg-gray-800 rounded-2xl max-w-[440px] w-full max-h-[90vh] overflow-y-auto flex flex-col shadow-2xl transform transition-all"
		...
		>
			...
			<div class="p-4 border-t border-gray-100 dark:border-gray-700 grid grid-cols-2 gap-3">
				...
				>
					...
					{:else}
						<Icon icon="material-symbols:link" is:inline />
